  ...
  *(UINT16*)EfiLoadOptionPtr = FilePathListLength;
  ...
  if ((BootType == BDS_LOADER_KERNEL_LINUX_ATAG) ||
    ...
    if (SrcLinuxArguments->InitrdSize > 0) {
      InitrdPathListPtr = (UINT8*)((EFI_PHYSICAL_ADDRESS)(DestLinuxArguments + 1) + SrcLinuxArguments->CmdLineSize);
      CopyMem (InitrdPathListPtr, (VOID*)((EFI_PHYSICAL_ADDRESS)(SrcLinuxArguments + 1) + SrcLinuxArguments->CmdLineSize), SrcLinuxArguments->InitrdSize);
    }
  }
