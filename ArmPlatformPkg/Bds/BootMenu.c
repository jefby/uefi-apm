  ...
  )
{
  ...
  if ((BootType == BDS_LOADER_KERNEL_LINUX_ATAG) ||
    ...
    CopyMem ((VOID*)((EFI_PHYSICAL_ADDRESS)(&BootArguments->LinuxArguments + 1) + CmdLineSize), InitrdPath, InitrdSize);
  } else {
    BootArguments = NULL;
  }
  ...
  if ((BootType == BDS_LOADER_KERNEL_LINUX_ATAG) ||
    ...
    if (InitrdSupport) {
      if (InitrdSize > 0) {
        ...
        Status = DeviceSupport->UpdateDevicePathNode ((EFI_DEVICE_PATH*)((EFI_PHYSICAL_ADDRESS)(LinuxArguments + 1) + CmdLineSize), L"initrd", &InitrdPath, NULL, NULL);
        if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {// EFI_NOT_FOUND is returned on empty input string, but we can boot without an initrd
          Status = EFI_ABORTED;
          goto EXIT;
        }
          ...
          if ((LoaderType == BDS_LOADER_KERNEL_LINUX_ATAG) ||
            ...
            if (ReadUnaligned16 (&OptionalData->Arguments.LinuxArguments.InitrdSize) > 0) {
              ...
              DevicePathTxt = DevicePathToTextProtocol->ConvertDevicePathToText (
                  GetAlignedDevicePath ((EFI_DEVICE_PATH*)((EFI_PHYSICAL_ADDRESS)(&OptionalData->Arguments.LinuxArguments + 1) + CmdLineSize)), TRUE, TRUE);
              Print(L"\t- Initrd: %s\n", DevicePathTxt);
            }
            Print(L"\t- Arguments: %a\n", (&OptionalData->Arguments.LinuxArguments + 1));
          }
