/**
 * This module implements PCIe functionality for APM XGene SoC.
...
static void xgene_pcie_serdes_pipe_config(void *base)
{
  ...
  if(apm88xxx_chip_revision() > APMXGeneRevA1) {
    //130830 added to remove SD glitch
    xgene_pcie_phy_rd(base, PIPE_REGS_LOS_PARAM, &dt);
    dt = CNTON_GEN12_SET(dt, 0xf);
    dt = CNTOFF_GEN12_SET(dt, 0x8);
    dt = CNTON_GEN3_SET(dt, 0xf);
    dt = CNTOFF_GEN3_SET(dt, 0x8);
    xgene_pcie_phy_wr(base, PIPE_REGS_LOS_PARAM, dt);

    //undocumented PIPE register, used for GEN1/GEN2 TX coefficiendterride
    xgene_pcie_phy_wr(base, (0x10000 | 0x0c), 0x2c0);

    //EBUF setting change
    xgene_pcie_phy_rd(base,  PIPE_REGS_EFIFO_CONTROL0, &dt);
    dt = BUF_DEPTH_PCI_SET(dt, 0x1a);
    xgene_pcie_phy_wr(base, PIPE_REGS_EFIFO_CONTROL0, dt);

    xgene_pcie_phy_rd(base, PIPE_REGS_PIPE_CONTROL, &dt);
    dt = WMSELECT_SET(dt, 0x6);
    xgene_pcie_phy_wr(base, PIPE_REGS_PIPE_CONTROL, dt);
  }
}
...
static void xgene_pcie_serdes_cfg_tx_ctrl(void *base, u32 ch, u32 sds2)
{
  ...
  if(apm88xxx_chip_revision() > APMXGeneRevA1) {
    //20130732: receiver detect change
...
static void xgene_pcie_serdes_cfg_rx_ctrl(void *base, u32 ch, u32 sds2)
{
  ...
  if(apm88xxx_chip_revision() > APMXGeneRevA1) {
    ...
    dt = CTLE_EQ_FR_SET(dt, 0x4);
    dt = CTLE_EQ_QR_SET(dt, 0x4);
    dt = CTLE_EQ_HR_SET(dt, 0x4);
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    if(apm88xxx_chip_revision() > APMXGeneRevA1) {
      ...
      dt = MU_PHASE1_SET(dt, 0x5);
      dt = MU_PHASE2_SET(dt, 0x5);
      dt = MU_PHASE3_SET(dt, 0x5);
    }
    xgene_pcie_phy_wr(base, addr + (CH_OFF * ch) + offset, dt);
    addr += 2;
  }
...
static void xgene_pcie_pll_calib(void *base, u32 link_width)
{
  ...
  if(apm88xxx_chip_revision() == APMXGeneRevA3) {
    /* Only required for A1 and A2 */
    for (j = 0; j < (link_width / 4); j++) {
      addr = j * SDS1_OFF;
...
static void xgene_pcie_manual_calib(void *base, u32 link_width)
{
  ...
  if (link_width == 8) {
    ...
    for (i = 0; i < 4; i++){
      ...
      xgene_pcie_phy_rd(base, CMU_REG17 + addr, &dt);
      dt = RESERVED_7_SET(dt, i);  //need to get updated spec from KC
      xgene_pcie_phy_wr(base,  CMU_REG17 + addr, dt);
...
static void xgene_pcie_adjust_phy_param(struct xgene_pcie_port *port)
{
  ...
  xgene_pcie_in32(base + PIPECTLREG, &dt);
  if (apm88xxx_chip_revision() == APMXGeneRevA1) {
    dt = PIPECTLREG_PHY_EQ_TX_FS_SET(dt, 0x2c);
    dt = PIPECTLREG_PHY_EQ_TX_LF_SET(dt, 0x0c);
    dt = PIPECTLREG_PHY_EQ_TX_MAX_PRE_SET(dt, 0x0b);
    dt = PIPECTLREG_PHY_EQ_TX_MAX_POST_SET(dt, 0x16);
  ...
  /* REVIEW PARAM */
  if (apm88xxx_chip_revision() == APMXGeneRevA1) {
    dt = CUSTOMER_PIN_MODE_SET(dt, 0x0d21);
  } else {
...
static void xgene_pcie_sds_rx_invert(void *base, int link_width)
{
  u32 data, i, j;
  u32 addr = 0;

  if (link_width < 4) {
    xgene_pcie_phy_rd(base, CH0_RXTX_REG13, &data);
    data = data | 0x2000;
    xgene_pcie_phy_wr(base, CH0_RXTX_REG13, data);
  } else {
    for (j = 0; j < link_width / 2; j++) {
      if (j == 1)
        addr = SDS1_OFF;
      for (i = 0 ; i < 4; i++) {
        xgene_pcie_phy_rd(base, CH0_RXTX_REG13 +
            (CH_OFF * i) + addr, &data);
        data = data | 0x2000;
        xgene_pcie_phy_wr(base, CH0_RXTX_REG13 +
            (CH_OFF * i) + addr, data);
      }
    }
  }
...
static void xgene_pcie_init_phy(struct xgene_pcie_port *port)
{
  ...
  if (apm88xxx_chip_revision() == APMXGeneRevA1)
    xgene_pcie_sds_rx_invert(base, link_width);
}
