...
VOID SetupL1Entries(VOID)
{
  ...
  *((UINT64 *)L1TTBAddr0) = (L2TTBAddr | 0x3);
   ...
   */
  for (i=0; i<510; i++) {
    AddL1BlockEntry(0x8080000000ULL + (i<<30), MEM_ATTR_DEVICE_SO);
  }
...
extern int dram_hw_init(unsigned long long *rambase, unsigned long long *ramsize);
...
RETURN_STATUS APMXGeneMemcInit(VOID)
{
  ...
  *MemSizeAddr = GetDDRSize();
...
#else
  UINT64 SystemRamBase;
  UINT64 SystemRamSize;
  ...
  dram_hw_init(&SystemRamBase, &SystemRamSize);

  MMUDisable();

  MemSizeAddr = (UINT64 *)PcdGet64(PcdMemSizeAddr);
  *MemSizeAddr =  SystemRamSize;
#endif
