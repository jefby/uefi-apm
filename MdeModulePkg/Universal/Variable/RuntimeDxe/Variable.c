  ...
  )
{
  ...
  //
  LinearOffset  = (EFI_PHYSICAL_ADDRESS)FwVolHeader;
  ...
  )
{
  ...
  while (IsValidVariableHeader (Variable)) {
    ...
    if (Variable->State == VAR_ADDED ||
      ...
      VariableSize = (UINTN)((EFI_PHYSICAL_ADDRESS)NextVariable - (EFI_PHYSICAL_ADDRESS)Variable);
  ...
  //
  if (UpdatingVariable != NULL) {
    VariableSize = (UINTN)((EFI_PHYSICAL_ADDRESS)(GetNextVariablePtr (UpdatingVariable)) - (EFI_PHYSICAL_ADDRESS)UpdatingVariable);
  ...
  )
{
  ...
  //
  NextVariable = GetEndPointer ((VARIABLE_STORE_HEADER *) (mVariableModuleGlobal->VariableGlobal.VolatileVariableBase));
  ...
  CopyMem (
    (UINT8 *) ((EFI_PHYSICAL_ADDRESS)NextVariable + VarNameOffset),
    VariableName,
    VarNameSize
    );
  VarDataOffset = VarNameOffset + VarNameSize + GET_PAD_SIZE (VarNameSize);
  CopyMem (
    (UINT8 *) ((EFI_PHYSICAL_ADDRESS)NextVariable + VarDataOffset),
    Data,
    DataSize
    );
  ...
  )
{
  ...
  NvStorageData = AllocateRuntimePool (NvStorageSize);
