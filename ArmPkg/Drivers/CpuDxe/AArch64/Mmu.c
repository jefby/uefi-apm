  ...
  )
{
  ...
  for (Index = 0; Index < EntryCount; Index++) {
    ...
    if ((EntryType == TT_TYPE_BLOCK_ENTRY) ||
      ...
      if ((*PrevEntryAttribute == TT_ATTR_INDX_INVALID) || (EntryAttribute != *PrevEntryAttribute)) {
        if (*PrevEntryAttribute != TT_ATTR_INDX_INVALID) {
          ...
          SetGcdMemorySpaceAttributes (MemorySpaceMap, NumberOfDescriptors,
              *StartGcdRegion,
              (BaseAddress + (Index * TT_BLOCK_ENTRY_SIZE_AT_LEVEL(TableLevel))) - *StartGcdRegion,
              PageAttributeToGcdAttribute (EntryAttribute));
        }

        // Start of the new region
        *StartGcdRegion = BaseAddress + (Index * TT_BLOCK_ENTRY_SIZE_AT_LEVEL(TableLevel));
      ...
      GetNextEntryAttribute ((UINT64*)(Entry & TT_ADDRESS_MASK_DESCRIPTION_TABLE),
                             ...
                             (BaseAddress + (Index * TT_BLOCK_ENTRY_SIZE_AT_LEVEL(TableLevel))),
  ...
  return BaseAddress + (EntryCount * TT_BLOCK_ENTRY_SIZE_AT_LEVEL(TableLevel));
}
