  ...
  )
{
  ...
  */
  if ((UINTN)FdtBlobBase > LINUX_ATAG_MAX_OFFSET) {
  ...
  if ((UINTN)InitrdAddress != LINUX_INITRD_MAX_OFFSET) {
    DEBUG((EFI_D_ERROR, "Relocate initrd from 0x%16LX to 0x%16LX\n",
          InitrdAddress, LINUX_INITRD_MAX_OFFSET));
    InitrdAddress = (EFI_PHYSICAL_ADDRESS) CopyMem(
                            (VOID *) LINUX_INITRD_MAX_OFFSET,
                            (VOID *) InitrdAddress,
                            InitrdSize);
  }
  ...
  IN  CONST CHAR8*              Arguments)
{
  ...
  // Load the Linux kernel from a device path
  //  Default below 128MB of DDR or any addr if no allocateable memory below 128MB
