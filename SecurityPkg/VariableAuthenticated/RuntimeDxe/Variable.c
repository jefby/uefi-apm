  ...
  )
{
  ...
  UINTN                       BlockIndex2;
  EFI_PHYSICAL_ADDRESS        LinearOffset;
  UINTN                       CurrWriteSize;
  EFI_PHYSICAL_ADDRESS        CurrWritePtr;
  UINT8                       *CurrBuffer;
  EFI_LBA                     LbaNumber;
  UINTN                       Size;
  ...
  if (!Volatile) {
    ...
    if ((DataPtr + DataSize) >= ((EFI_PHYSICAL_ADDRESS) ((UINT8 *) FwVolHeader + FwVolHeader->FvLength))) {
      return EFI_INVALID_PARAMETER;
    }
  } else {
    //
    ...
    // written.
    //
    VolatileBase = (VARIABLE_STORE_HEADER *) (mVariableModuleGlobal->VariableGlobal.VolatileVariableBase);
    if (SetByIndex) {
      DataPtr += mVariableModuleGlobal->VariableGlobal.VolatileVariableBase;
    }

    if ((DataPtr + DataSize) >= (((UINT8 *) VolatileBase + VolatileBase->Size))) {
      return EFI_INVALID_PARAMETER;
    }
    ...
    //
    CopyMem ((UINT8 *)DataPtr, Buffer, DataSize);
    return EFI_SUCCESS;
  }
  ...
  //
  LinearOffset  = FwVolHeader;
  CurrWritePtr  = DataPtr;
  ...
  )
{
  ...
  //
  Value =  (EFI_PHYSICAL_ADDRESS)GetVariableNamePtr (Variable);
  ...
  )
{
  ...
  Value =  (EFI_PHYSICAL_ADDRESS)GetVariableDataPtr (Variable);
  ...
  )
{
  ...
  // The end of variable store
  //
  return (VARIABLE_HEADER *) HEADER_ALIGN ((EFI_PHYSICAL_ADDRESS)VarStoreHeader + VarStoreHeader->Size);
}
  ...
  )
{
  ...
  while (IsValidVariableHeader (Variable)) {
    ...
    if (Variable->State == VAR_ADDED ||
      ...
      VariableSize = (EFI_PHYSICAL_ADDRESS)NextVariable - (EFI_PHYSICAL_ADDRESS)Variable;
  ...
  if (ReclaimPubKeyStore) {
    ...
    //
    if (UpdatingVariable != NULL) {
      VariableSize = ((EFI_PHYSICAL_ADDRESS)GetNextVariablePtr (UpdatingVariable)) - (EFI_PHYSICAL_ADDRESS)UpdatingVariable;
  ...
  if (IsVolatile) {
    ...
    //
    SetMem ((UINT8 *) VariableBase, VariableStoreHeader->Size, 0xff);
    CopyMem ((UINT8 *) VariableBase, ValidBuffer, (UINTN) (CurrPtr - (UINT8 *) ValidBuffer));
    Status  = EFI_SUCCESS;
  } else {
    //
    ...
    CopyMem (mNvVariableCache, (CHAR8 *)VariableBase, VariableStoreHeader->Size);
  }
  if (!EFI_ERROR (Status)) {
  ...
  )
{
  ...
  VariableStoreHeader[VariableStoreTypeHob]      = (VARIABLE_STORE_HEADER *) Global->HobVariableBase;
  ...
  )
{
  ...
  if ((CacheVariable->CurrPtr == NULL) || CacheVariable->Volatile) {
    ...
    //
    VariableStoreHeader  = (VARIABLE_STORE_HEADER *) (mVariableModuleGlobal->VariableGlobal.NonVolatileVariableBase);
    Variable = &NvVariable;
    Variable->StartPtr = GetStartPointer (VariableStoreHeader);
    Variable->EndPtr   = GetEndPointer (VariableStoreHeader);
    Variable->CurrPtr  = (VARIABLE_HEADER *)((EFI_PHYSICAL_ADDRESS)Variable->StartPtr + ((EFI_PHYSICAL_ADDRESS)CacheVariable->CurrPtr - (EFI_PHYSICAL_ADDRESS)CacheVariable->StartPtr));
  ...
  if (Variable->CurrPtr != NULL) {
    ...
    if (DataSizeOfVariable (Variable->CurrPtr) == DataSize &&
      ...
      if ((Attributes & EFI_VARIABLE_APPEND_WRITE) != 0) {
        ...
        CopyMem (mStorageArea, (UINT8*)(Variable->CurrPtr + DataOffset), Variable->CurrPtr->DataSize);
        //
        ...
        if ((CompareGuid (VendorGuid, &gEfiImageSecurityDatabaseGuid) &&
          ...
          CopyMem ((UINT8*)(mStorageArea + Variable->CurrPtr->DataSize), Data, DataSize);
  ...
  CopyMem (
    (UINT8 *) (NextVariable + VarNameOffset),
    VariableName,
    VarNameSize
    );
  ...
  if ((Attributes & EFI_VARIABLE_NON_VOLATILE) != 0) {
    ...
    //
    Volatile = FALSE;
    NonVolatileVarableStoreSize = ((VARIABLE_STORE_HEADER *)(mVariableModuleGlobal->VariableGlobal.NonVolatileVariableBase))->Size;
    ...
    if ((UINT32) (VarSize + mVariableModuleGlobal->VolatileLastVariableOffset) >
        ((VARIABLE_STORE_HEADER *) ((mVariableModuleGlobal->VariableGlobal.VolatileVariableBase)))->Size) {
      //
      // Perform garbage collection & reclaim operation.
      //
      Status = Reclaim (
  ...
  )
{
  ...
  //
  VariableStoreHeader[VariableStoreTypeVolatile] = (VARIABLE_STORE_HEADER *)mVariableModuleGlobal->VariableGlobal.VolatileVariableBase;
  VariableStoreHeader[VariableStoreTypeHob]      = (VARIABLE_STORE_HEADER *)mVariableModuleGlobal->VariableGlobal.HobVariableBase;
  VariableStoreHeader[VariableStoreTypeNv]       = mNvVariableCache;
  ...
  )
{
  ...
  if (1 < InterlockedIncrement (&mVariableModuleGlobal->VariableGlobal.ReentrantState)) {
    ...
    //
    NextVariable  = GetStartPointer ((VARIABLE_STORE_HEADER *)Point);
    while ((NextVariable < GetEndPointer ((VARIABLE_STORE_HEADER *)Point))
        && IsValidVariableHeader (NextVariable)) {
      NextVariable = GetNextVariablePtr (NextVariable);
    }
    mVariableModuleGlobal->NonVolatileLastVariableOffset = (EFI_PHYSICAL_ADDRESS)NextVariable - (EFI_PHYSICAL_ADDRESS)Point;
  }
  ...
  )
{
  ...
  if((Attributes & EFI_VARIABLE_NON_VOLATILE) == 0) {
    ...
    //
    VariableStoreHeader = (VARIABLE_STORE_HEADER *) (mVariableModuleGlobal->VariableGlobal.VolatileVariableBase);
  } else {
    //
  ...
  )
{
  ...
  CommonVariableSpace = ((VARIABLE_STORE_HEADER *) ((mVariableModuleGlobal->VariableGlobal.NonVolatileVariableBase)))->Size - sizeof (VARIABLE_STORE_HEADER) - PcdGet32(PcdHwErrStorageSize); //Allowable max size of common variable storage space
  ...
  )
{
  ...
  //
  CopyMem (NvStorageData, (UINT8 *)NvStorageBase, NvStorageSize);
  ...
  if (GuidHob != NULL) {
    ...
    if (FtwLastWriteData->TargetAddress == NvStorageBase) {
      ...
      //
      CopyMem (NvStorageData, (UINT8 *)(FtwLastWriteData->SpareAddress), NvStorageSize);
      ...
      //
      CopyMem (NvStorageData + BackUpOffset, (UINT8 *)FtwLastWriteData->SpareAddress, BackUpSize);
    }
  }
  ...
  VariableStoreBase = (EFI_PHYSICAL_ADDRESS) (FvHeader + FvHeader->HeaderLength);
  VariableStoreLength = (UINT64) (NvStorageSize - FvHeader->HeaderLength);

  mVariableModuleGlobal->VariableGlobal.NonVolatileVariableBase = VariableStoreBase;
  mNvVariableCache = (VARIABLE_STORE_HEADER *)VariableStoreBase;
  ...
  // Parse non-volatile variable data and get last variable offset.
  //
  NextVariable  = GetStartPointer ((VARIABLE_STORE_HEADER *)VariableStoreBase);
  while (IsValidVariableHeader (NextVariable)) {
    VariableSize = NextVariable->NameSize + NextVariable->DataSize + sizeof (VARIABLE_HEADER);
  ...
  mVariableModuleGlobal->NonVolatileLastVariableOffset = (EFI_PHYSICAL_ADDRESS)NextVariable -  (EFI_PHYSICAL_ADDRESS)VariableStoreBase;
  ...
  )
{
  ...
  if (mVariableModuleGlobal->VariableGlobal.HobVariableBase != 0) {
    VariableStoreHeader = (VARIABLE_STORE_HEADER *)mVariableModuleGlobal->VariableGlobal.HobVariableBase;
    //
  ...
  )
{
  ...
  //
  mVariableModuleGlobal->VariableGlobal.NonVolatileVariableBase = VariableStoreBase;
  VariableStoreHeader = (VARIABLE_STORE_HEADER *)VariableStoreBase;
  ...
  )
{
  ...
  if (GuidHob != NULL) {
    ...
    if (GetVariableStoreStatus (VariableStoreHeader) == EfiValid) {
      mVariableModuleGlobal->VariableGlobal.HobVariableBase = (EFI_PHYSICAL_ADDRESS)AllocateRuntimeCopyPool (VariableStoreLength, (VOID *) VariableStoreHeader);
      if (mVariableModuleGlobal->VariableGlobal.HobVariableBase == 0) {
  ...
  //
  mVariableModuleGlobal->VariableGlobal.VolatileVariableBase = (EFI_PHYSICAL_ADDRESS) VolatileVariableStore;
  mVariableModuleGlobal->VolatileLastVariableOffset = (UINTN)((EFI_PHYSICAL_ADDRESS)GetStartPointer (VolatileVariableStore) - (EFI_PHYSICAL_ADDRESS)VolatileVariableStore);
